            ...
            {careerItems.map((item, index) => <div key={`${item.company}-${index}`} className={`relative flex flex-col md:flex-row md:items-stretch gap-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                ...
                <div className={`hidden md:flex md:flex-col md:w-[calc(50%-2rem)] ${index % 2 === 0 ? 'md:pl-8' : 'md:pr-8'} ${item.logo || item.image || item.images ? 'md:gap-4' : ''}`}>
                  ...
                  {item.images && item.images.map((img, imgIndex) => {
                    const imageLink = item.imageLinks?.[imgIndex] ?? item.link;
                    const isConsulateSecondImage = item.company === "Consulate General of the Republic of Indonesia in Los Angeles" && imgIndex === 1;
                    return imageLink ? (
                      <a key={imgIndex} href={imageLink} target="_blank" rel="noopener noreferrer" className="overflow-hidden rounded-2xl group">
                        <img src={img} alt={`${item.company} work ${imgIndex + 1}`} className={`w-full h-48 rounded-2xl border border-border hover:border-primary/50 transition-all duration-300 group-hover:scale-105 ${item.company === "Adobe Inc" || (item.company === "Lucasfilm Ltd (Industrial Light & Magic)" && imgIndex === 0) || item.company === "Warner Bros Studios" || item.company === "PT MultiMedika International, Tbk" || (item.company === "Wander Social" && imgIndex === 1) || isConsulateSecondImage ? "object-cover" : "object-contain bg-card p-2"} ${item.company === "Wander Social" && imgIndex === 1 ? "object-bottom" : "object-center"} ${isConsulateSecondImage ? "scale-150 object-top" : ""}`} />
                      </a>
                    ) : (
