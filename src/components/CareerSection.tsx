          ...
          <div className="space-y-12 shadow-none">
            {careerItems.map((item, index) => <div key={`${item.company}-${index}`} className={`relative flex flex-col md:flex-row md:items-stretch gap-8 ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'}`}>
                ...
                <div className={`hidden md:flex md:flex-col md:w-[calc(50%-2rem)] ${index % 2 === 0 ? 'md:pl-8' : 'md:pr-8'} ${item.logo || item.image || item.images ? 'md:gap-4' : ''}`}>
                  ...
                  {item.image && <img src={item.image} alt={`${item.company} work`} className={`w-full object-contain object-center rounded-2xl border border-border hover:border-primary/50 transition-all duration-300 bg-card p-2 ${item.logo ? 'h-48' : 'h-full'}`} />}
                  {item.images && item.images.map((img, imgIndex) => (
                    <img key={imgIndex} src={img} alt={`${item.company} work ${imgIndex + 1}`} className={`w-full h-48 object-center rounded-2xl border border-border hover:border-primary/50 transition-all duration-300 ${imgIndex === 0 && item.company === "Adobe Inc" ? "object-cover" : "object-contain bg-card p-2"}`} />
                  ))}
                </div>
              </div>)}
          </div>
